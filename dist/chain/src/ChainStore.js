...
var ChainStore = function () {
    ...
    ChainStore.prototype.init = function init() {
        ...
        var _init = function _init(resolve, reject) {
            ...
            var db_api = _idacjsWs.Apis.instance().db_api();
            if (!db_api) {
            ...
            return db_api.exec("get_objects", [["2.1.0"]]).then(function (optional_objects) {
                ...
                for (var i = 0; i < optional_objects.length; i++) {
                    ...
                    if (optional_object) {
                        ...
                        if (delta < 100) {
                            _idacjsWs.Apis.instance().db_api().exec("set_subscribe_callback", [_this.onUpdate.bind(_this), true]).then(function () {
    ...
    ChainStore.prototype.getAsset = function getAsset(id_or_symbol) {
        ...
        _idacjsWs.Apis.instance().db_api().exec("lookup_asset_symbols", [[id_or_symbol]]).then(function (asset_objects) {
    ...
    ChainStore.prototype.getAccountRefsOfKey = function getAccountRefsOfKey(key) {
        ...
        if (this.get_account_refs_of_keys_calls.has(key)) return this.account_ids_by_key.get(key);else {
            ...
            _idacjsWs.Apis.instance().db_api().exec("get_key_references", [[key]]).then(function (vec_account_id) {
    ...
    ChainStore.prototype.getBalanceObjects = function getBalanceObjects(address) {
        ...
        if (current === undefined) {
            ...
            _idacjsWs.Apis.instance().db_api().exec("get_balance_objects", [[address]]).then(function (balance_objects) {
                var set = new Set();
    ...
    ChainStore.prototype.fetchObject = function fetchObject(id) {
        ...
        if (result === undefined) {
            ...
            _idacjsWs.Apis.instance().db_api().exec("get_objects", [[id]]).then(function (optional_objects) {
                //if(DEBUG) console.log("... optional_objects",optional_objects ? optional_objects[0].id : null)
    ...
    ChainStore.prototype.fetchWitnessByAccount = function fetchWitnessByAccount(account_id) {
        ...
        return new Promise(function (resolve, reject) {
            _idacjsWs.Apis.instance().db_api().exec("get_witness_by_account", [account_id]).then(function (optional_witness_object) {
    ...
    ChainStore.prototype.fetchCommitteeMemberByAccount = function fetchCommitteeMemberByAccount(account_id) {
        ...
        return new Promise(function (resolve, reject) {
            _idacjsWs.Apis.instance().db_api().exec("get_committee_member_by_account", [account_id]).then(function (optional_committee_object) {
    ...
    ChainStore.prototype.fetchFullAccount = function fetchFullAccount(name_or_id) {
        ...
        if (!this.fetching_get_full_accounts.has(name_or_id) || Date.now() - this.fetching_get_full_accounts.get(name_or_id) > 5000) {
            ...
            _idacjsWs.Apis.instance().db_api().exec("get_full_accounts", [[name_or_id], true]).then(function (results) {
                if (results.length === 0) {
                    if (_ChainValidation2.default.is_object_id(name_or_id)) {
                ...
                if (sub_to_objects.length) _idacjsWs.Apis.instance().db_api().exec("get_objects", [sub_to_objects]);
    ...
    ChainStore.prototype.fetchRecentHistory = function fetchRecentHistory(account) {
        ...
        pending_request.promise = new Promise(function (resolve, reject) {
            _idacjsWs.Apis.instance().history_api().exec("get_account_history", [account_id, most_recent, limit, start]).then(function (operations) {
        ...
        if (object.id.substring(0, balance_prefix.length) == balance_prefix) {
            ...
            if (account) {
                ...
                if (!call_orders.has(object.id)) {
                    ...
                    this.objects_by_id.set(account.get("id"), account);
                    _idacjsWs.Apis.instance().db_api().exec("get_objects", [[object.id]]); // Force subscription to the object in the witness node by calling get_objects
                }
            }
        } else if (object.id.substring(0, order_prefix.length) == order_prefix) {
            // LIMIT ORDER OBJECT
    ...
    ChainStore.prototype.getObjectsByVoteIds = function getObjectsByVoteIds(vote_ids) {
        ...
        if (missing.length) {
            ...
            _idacjsWs.Apis.instance().db_api().exec("lookup_vote_ids", [missing]).then(function (vote_obj_array) {
