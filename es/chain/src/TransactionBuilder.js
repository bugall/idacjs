...
import { Apis, ChainConfig } from 'idacjs-ws';
...
function _broadcast(was_broadcast_callback) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        Apis.instance().network_api().exec("broadcast_transaction_with_callback", [function (res) {
            ...
            reject(new Error(message + "\n" + 'idac-crypto ' + ' digest ' + hash.sha256(_this5.tr_buffer).toString('hex') + ' transaction ' + _this5.tr_buffer.toString('hex') + ' ' + JSON.stringify(tr_object)));
            return;
        });
        return;
    });
